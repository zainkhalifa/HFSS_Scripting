...
% CP_4PJunction generates a script to draws a coplaner 4-Junction then you 
% can move and rotate. 
% 
% Author : Zainulabideen Khalifa            Last Revision : 10/01/2020
% 
...
% function [Names] = CP_4PJunction(fileID,label,Line,Shield)
    ...
    Attrib.name = strcat(label,"Sd"); Names{1+end} = Attrib.name;
    Pars.x = strcat("(",Line.length,")");
    Pars.y = "0";
    Pars.z = Shield.elevation;
    Pars.dx = Shield.width;
